...
def push_image():
    ...
    if request.method == 'POST':
        ...
        if isinstance(post_request_info, dict):
            ...
            if '/' in jfrog_image_name:
                return 'jfrog_image_name: {jfrogImgName} format is Error'.format(jfrogImgName=jfrog_image_name)
            ...
            """ 
            1. 定义 ips_result 列表，主要存储返回信息. 
            2. 定义 prod_registry_ips 列表,主要存储生产 registry_ip.
            3. prod_registry_url 变量存在则执行 else 部分仅 push 到指定的仓库, 如果不存在，push到 prod_registry_ips 列表中所有仓库
            """
            prod_registry_ip1 = os.getenv('registry_ip1')
            prod_registry_ip2 = os.getenv('registry_ip2')
            prod_registry_ip3 = os.getenv('registry_ip3')
            prod_registry_ip4 = os.getenv('registry_ip4')
            prod_registry_ip5 = os.getenv('registry_ip5')
            prod_registry_ip6 = os.getenv('registry_ip6')

            prod_registry_ips = ['{ip1}'.format(ip1=prod_registry_ip1), '{ip2}'.format(ip2=prod_registry_ip2),
                                 '{ip3}'.format(ip3=prod_registry_ip3), '{ip4}'.format(ip4=prod_registry_ip4),
                                 '{ip5}'.format(ip5=prod_registry_ip5), '{ip6}'.format(ip6=prod_registry_ip6)]
            if prod_registry_url is None:
                for ip_index in range(0, len(prod_registry_ips)):
                    ...
                    if client.api.tag(jfrogImginfo, repository='{prodImg}'.format(prodImg=prodimagetag),
                        ...
                        ips_result.append(
                            {'Response': 404, 'Info': 'ImageTagChangeFailed',
                             'ip': prod_registry_ips[ip_index]})
            else:
